...
argv <- add_argument(argv, "-i", help="input_file")
...
#input_file<-"results/CRTISO_RNP_optimization.tsv"
...
for (mytarget_i in c("CRTISO_49and50bp","CRTISO","Psy1","PhyB2.1","PhyB2.2","PhyB2.3"))
	{
	for (myinduction in c("gRNA","RNP"))
  		...
  		if (mytarget=="CRTISO_49and50bp") {mydata<-read.csv(paste0("csv/CRTISO_",myinduction,"_49and50PerfectDSB_Types_MH_df.csv"));mytarget<-"CRTISO"} else 
		if (nophyB2)
			{
    			mytarget<-mytarget_i
    			mydata<-read.csv(paste0("csv/",mytarget,"_",myinduction,"_Types_MH_df.csv"))
			};
            	if (mytarget=="CRTISO" && myinduction=="gRNA") { mydata<-mydata[mydata$X!="CRTISO_6h_A",]};
            	if (mytarget=="CRTISO" && myinduction=="RNP") { mydata<-mydata[mydata$X!="CRTISO_0h_B",];mydata<-mydata[1:11,]};
		if (nophyB2)
			{
			mydata[is.na(mydata)]<-0; mydata<-cbind(matrix(unlist(sapply(as.character(mydata$X),function(x) strsplit(x,"_"))),ncol=3,byrow=T),mydata); names(mydata)[1:4]<-c("target","time","replicate","ID_name");               mydata$time<-as.numeric(sapply(mydata$time, function(x) gsub("h","",x)))
...
myerror<-inputf$error[1]
mymodel<-inputf$model[1]
print(c("working on:",mytarget_i,myinduction,myerror,mymodel))
  ...
  if (inputasrates)
    ...
    for (xrate in nameparams)
       ...
       counter00<-1
        ...
        #mysd<-sapply(xxparams,function(z) if (z==0) return(0.001) else return(z/exploration.radius/1000))
        ...
        #mysd<-sapply(xxparams,function(z) if (z==0) return(0.001) else return(z/exploration.radius/10))
        ...
        #mysd<-sapply(xxparams,function(z) if (z==0) return(0.001) else return(z/exploration.radius))
        ...
        #mysd<-sapply(xxparams,function(z) if (z==0) return(0.001) else return(z/exploration.radius*100))
...
res<-generate_CI(bestmodel,inputasrates=FALSE,likfunction=function(zz) loglik_er_f.pen(zz,my_data=mydata_l[[paste0(myinduction,"_",mytarget_i)]],ODEfunc=get(mymodel),E.matrix=error_mat_l[[mytarget_i]][[myerror]]),npermutations=n.max,returnonlyCI=TRUE,addpreviouslysampled="inputf")
