...
for ( counter in 1:n.max)
	...
	names(xparms)<-nameparms
        ...
        loglik_er_f.pen(z,my_data=mydata,ODEfunc=xmodel,E.matrix=error_mat_l[[mytarget_i]][[myerror]]),control=list(maximize=TRUE,maxit=1000,parscale=myparscale,ndeps=mysteps,lmm=mylmm,factr=myfactr),method="L-BFGS-B",lower=lower_bound,hessian=FALSE,gr=NULL) #dowarn=FALSE
	names(myparscale)<-paste0("parscale",1:nrates)
        names(xparms)<-paste0("init_",names(xparms))
        names(mysteps)<-paste0("step",1:nrates)
        res_allinfo<-t(unlist(list(mytarget_i,myinduction,mymodel,myerror,res,xparms,myparscale,mysteps)))
