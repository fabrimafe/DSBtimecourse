...
for MYINDUCTION_CURVE in 2;do # 3 4; do # 2 4
for mymodel in modelDSBs1i1_3x4.bytarget;do #modelDSBs1i1_mini.bytarget modelDSBs1i1_nok12.bytarget; do 
for MYTARGET in Psy1_all PhyB2.2_all CRTISO_all;do # CRTISO.49and50bp_all;do # CRTISO.49and50bp_all;do #Psy1_R2_Feb2022;do #CRTISO.cleanedandnov2021 CRTISO.nov2021.49and50 Psy1 PhyB2.2.nov2021 CRTISO.nov2021; do 
...
for bootstraptype in stratifiedbootstraps2;do #stratifiedbootstraps;do #stationarybootstraps MEbootstraps; do #MEbootstraps stationarybootstraps
#for mymodel in #modelDSBs1i1_realimprecise;do #modelDSBs1i1_3x4 modelDSBs1i1_nok12 modelDSBs1i1_mini; do # modelDSBs1i1_3x4;do #modelDSBs1i1_mini modelDSBs1i1_nok12 modelDSBs1i1_mini modelDSBs1i1_3x4 modelDSBs1i1_nok12; do 
#for MYTARGET in CRTISO.49and50bp_allb;do #Psy1_allb PhyB2.2_allb;do #CRTISO_allb Psy1_allb PhyB2.2_allb;do #CRTISO.cleanedandnov2021;do
...
MYTARGETM=$( echo $MYTARGET | sed 's/_allb//' |  sed 's/_all//' | sed 's/0h//' | sed 's/m$//' | sed 's/72h//' )
ERRORMATRIX=~/workspace/daniela/error_matrices/error_matrix4_${MYTARGETM}_errorsfromunbroken.tsv
...
for MYINDUCTION_CURVE in 2; do #3 4; do # gRNA;do
...
#for mymodel in modelDSBs1i1_nok12 modelDSBs1i1_mini modelDSBs1i1_3x4; do #modelDSBs1i1_realimprecise model5i1 modelDSBs1i1_realimpnor11;do
#for MYTARGET in CRTISO_allb CRTISO.49and50bp_allb;do # Psy1_allb PhyB2.2_allb;do #PhyB2.2_R2_Feb2022 Psy1_R2_Feb2022 Psy1_allb PhyB2.2_allb CRITSO_allb CRTISO Psy1;do #CRTISO.nov2021.49and50 Psy1 PhyB2.2.nov2021 CRTISO.nov2021;do 
...
#stratifiedbootstraps2 goes with all and modelDSBs1i1_mini.bytarget; stratifiedbootstraps with allb
for MYN in 20000 50000;do # 500000;do # 150000 2000000
for MYDELAY in '';do # _mydelay0.25;do
for MYINDUCTION in RNP; do # gRNA;do
for MYINDUCTION_CURVE in 2;do # 3 4; do # 2 4
...
#for mymodel in modelDSBs1i1_3x4;do # modelDSBs1i1_mini modelDSBs1i1_nok12;do # modelDSBs1i1_3x4;do #modelDSBs1i1_mini modelDSBs1i1_nok12;do 
#for MYTARGET in Psy1_allb PhyB2.2_allb CRTISO_allb CRTISO.49and50bp_allb;do #CRTISO_allb Psy1_allb PhyB2.2_allb;do #CRTISO.cleanedandnov2021;do
...
bsub -q new-long -J CI -e ~/mylogs3/confint.e%J -o ~/mylogs3/confint.o%J -R rusage[mem=18000] ./calculate_CI_v1.sh ${OUTPUTA}.parsed.tab $TIMECOURSE $ERRORMATRIX $MYN $mymodel $likfun $MYINDUCTION_CURVE
...
for MYDELAY in '';do # _mydelay0.25;do #''
for MYINDUCTION in RNP;do #gRNA
for MYINDUCTION_CURVE in 2; do #3 gRNA;do
...
#for mymodel in modelDSBs1i1_nok12 modelDSBs1i1_3x4 modelDSBs1i1_mini;do
#for MYTARGET in Psy1_allb CRTISO_allb CRTISO.49and50bp_allb PhyB2.2_allb;do #PhyB2.2_R2_Feb2022 Psy1_R2_Feb2022 Psy1_allb PhyB2.2_allb CRITSO_allb; do #Psy1 PhyB2.2.nov2021 CRTISO.nov2021 CRTISO.cleanedandnov2021 CRTISO.nov2021.49and50 ;do 
MYTARGETM=$( echo $MYTARGET | sed 's/_allb//' |  sed 's/_all//' | sed 's/_R2_Feb2022//' | sed 's/0h//' | sed 's/m$//' | sed 's/72h//' )
ERRORMATRIX=~/workspace/daniela/error_matrices/error_matrix4_${MYTARGETM}_errorsfromunbroken.tsv
TIMECOURSE=~/workspace/daniela/input_datasets/timecourse_${MYINDUCTION}_${MYTARGET}${MYDELAY}.txt
likfun=0; if [ $mymodel == "modelDSBs1i1_3x4" ];then likfun=3; fi
...
#               bsub -q new-short -J plotshort -e ~/mylogs/plot.e%J -o ~/mylogs/plot.o%J -R rusage[mem=16000] ./plot.v1.sh $myfile $TIMECOURSE $ERRORMATRIX $mymodel 1000 ~/workspace/daniela/resultsv4/plots/plot_${mymodel}_${MYINDUCTION}_ind.c${MYINDUCTION_CURVE}_${MYTARGET}${MYDELAY} $likfun ${MYINDUCTION_CURVE} ${TIMERESOLUTION}
	bsub -q new-short -J plotshort -e ~/mylogs/plot.e%J -o ~/mylogs/plot.o%J -R rusage[mem=16000] ./plot.v2.sh $myfile $TIMECOURSE $ERRORMATRIX $mymodel 1000 ~/workspace/daniela/resultsv4/plots/plot_${mymodel}_${MYINDUCTION}_ind.c${MYINDUCTION_CURVE}_${MYTARGET}${MYDELAY} $likfun ${MYINDUCTION_CURVE} ${TIMERESOLUTION} 1
        fi
fi
...
#for mymodel in modelDSBs1i1_3x4;do # modelDSBs1i1_mini modelDSBs1i1_nok12;do 
#for MYTARGET in CRTISO.49and50bp_allb CRTISO_allb Psy1_allb PhyB2.2_allb;do #CRTISO.49and50bp_allb;do #CRTISO_allb Psy1_allb PhyB2.2_allb;do #CRTISO.cleanedandnov2021;do
...
listCIFILE=/home/labs/alevy/fabrizio/workspace/daniela/resultsv4/${bootstraptype}/timecourse_${MYINDUCTION}_${MYTARGET}/${mymodel}/results_${mymodel}_${MYINDUCTION}_ind.c${MYINDUCTION_CURVE}_${MYTARGET}/listCIfiles.txt
#myfiles=$( ls /home/labs/alevy/fabrizio/workspace/daniela/resultsv4/results_${mymodel}_${MYINDUCTION}_ind.c${MYINDUCTION_CURVE}_${MYTARGET}.all_n500000.CI /home/labs/alevy/fabrizio/workspace/daniela/resultsv4/${bootstraptype}/timecourse_${MYINDUCTION}_${MYTARGET}/${mymodel}/results_${mymodel}_${MYINDUCTION}_ind.c${MYINDUCTION_CURVE}_${MYTARGET}/results*.tab_n50000.CI )
#for ifile in $myfiles;do
#cat $ifile | awk -v OFS='\t' '{if (ar[$4]==1){$4=$4".1"}; ar[$4]=1; print $0}' > ${ifile}.t.CI 
#echo $ifile
#done
ls /home/labs/alevy/fabrizio/workspace/daniela/resultsv4/results_${mymodel}_${MYINDUCTION}_ind.c${MYINDUCTION_CURVE}_${MYTARGET}.all_n50000.CI.t.CI > $listCIFILE 
ls /home/labs/alevy/fabrizio/workspace/daniela/resultsv4/${bootstraptype}/timecourse_${MYINDUCTION}_${MYTARGET}/${mymodel}/results_${mymodel}_${MYINDUCTION}_ind.c${MYINDUCTION_CURVE}_${MYTARGET}/results*.tab_n20000.CI.t.CI >> $listCIFILE
